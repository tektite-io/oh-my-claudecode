 ...
 */
export function generateJsonReport(results, model) {
    ...
    const headToHead = fixtureIds.map((fixtureId) => {
...
export function generateMarkdownReport(report) {
    ...
    for (const fixtureId of fixtureIds) {
        ...
        lines.push('');
        for (const agentType of ['harsh-critic', 'critic']) {
            const result = report.results.find((r) => r.fixtureId === fixtureId && r.agentType === agentType);
            if (!result)
                continue;
            const s = result.scores;
            lines.push(`- **${agentType}**: composite=${pct(s.compositeScore)} ` +
                `tp=${pct(s.truePositiveRate)} fn=${pct(s.falseNegativeRate)} ` +
                `fp=${pct(s.falsePositiveRate)}`);
            lines.push(`  - Matched: ${result.matchedFindings.length}/${result.matchedFindings.length + result.missedFindings.length} findings`);
    ...
    let largestMetric = 'compositeScore';
    let largestDelta = 0;
    for (const key of SUMMARY_METRICS) {
    ...
    const label = METRIC_LABELS[largestMetric] ?? largestMetric;
    const direction = largestDelta >= 0 ? 'improved' : 'regressed';
    lines.push(`**${label}** showed the largest difference: harsh-critic ${direction} by ${sign(largestDelta)} over critic.`);
    lines.push('');
    return lines.join('\n');
}
